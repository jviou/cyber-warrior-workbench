...
export type Journal = {
  id?: number | string;
  kind: string; summary: string; details?: string;
  at?: string; actionId?: number | string;
};

export const Actions = {
  list: () => http<Action[]>('/actions?_sort=createdAt&_order=desc'),
  create: (a: Action) => http<Action>('/actions', {
    method: 'POST',
    body: JSON.stringify({ createdAt: new Date().toISOString(), status: 'todo', ...a }),
  }),
  update: (id: number | string, patch: Partial<Action>) => http<Action>(`/actions/${id}`, {
    method: 'PATCH',
...
export const JournalApi = {
  ...
  add:  (e: Journal) => http<Journal>('/journal', {
    method: 'POST',
...
export async function audit(entity: string, op: 'create'|'update'|'delete', before?: unknown, after?: unknown, actor='ui') {
  try {
    await http('/audit', { method: 'POST', body: JSON.stringify({ entity, op, before, after, ts: new Date().toISOString(), actor }) });
  } catch {}
}
